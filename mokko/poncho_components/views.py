...
def update_component_table(request):
    ...
    gl = gitlab.Gitlab('https://gitlab.anwbonline.nl/', private_token='eteXYxTEbF8bJJR__67A')
    project = gl.projects.get(238)
    componentlist = [(x['name'], x['path'] + '/src/') for x in project.repository_tree(path='packages/components', all=True)]
    for component in componentlist:
        ...
        if PonchoComponents.objects.filter(componentname = component[0]).count() == 0:
            ...
            c.save()
            print(c)
        print('url: ' + 'packages/components/'+component[1])
